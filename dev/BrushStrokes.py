# Old, deprecated brush stroke script 
# modified to work with Blender 2.78 and GP2 to read old json files

# "C:\\Users\\nick\\Documents\\GitHub\\LightningArtist\\latk_tools\\animation_archive\\old_format\\brushstrokes-saved2.json"
...
# oldest version; still image only
def readBrushStrokesStill(url=None):
    gp = getActiveGp()
    '''
    gp = bpy.data.grease_pencil.new("My GPencil")
    scene = bpy.context.scene
    ...
    for j in range(0, len(data["brushstrokes"])):
        stroke = frame.strokes.new(getActiveColor().name)
        stroke.draw_mode = "3DSPACE" # either of ("SCREEN", "3DSPACE", "2DSPACE", "2DIMAGE")
        stroke.points.add(len(data["brushstrokes"][j])) # add 4 points
        #stroke.points[0].co = (..., ..., ...) # set 1st point's location
        #stroke.points.foreach_set("co", (0,0,0,0,0,4,0,6,4,8,6,4)) # set all at once efficiently
        for l in range(0, len(data["brushstrokes"][j])):
            #print(data["brushstrokes"][i][j][l])
            x = 0.0
            y = 0.0
            z = 0.0
            if useScaleAndOffset == True:
                x = (data["brushstrokes"][j][l]["x"] * globalScale.x) + globalOffset.x
                y = (data["brushstrokes"][j][l]["z"] * globalScale.y) + globalOffset.y
                z = (data["brushstrokes"][j][l]["y"] * globalScale.z) + globalOffset.z
            else:
                x = data["brushstrokes"][j][l]["x"]
                y = data["brushstrokes"][j][l]["z"]
                z = data["brushstrokes"][j][l]["y"]
            #stroke.points[l].co = (x, y, z)
            createPoint(stroke, l, (x, y, z))

rb = readBrushStrokes
rbs = readBrushStrokesStill
wb = writeBrushStrokes

# removed to prevent conflicts with current utility functions in Latk
'''
# ~ ~ ~ ~ ~ ~ ~ ~ 
def roundVal(a, b):
    formatter = "{0:." + str(b) + "f}"
